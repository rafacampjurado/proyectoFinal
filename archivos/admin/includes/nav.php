<div id="navigation">



    <!-- container -->



    <div class="container">



        <div id="responsive-nav">



            <!-- category nav -->







            <!-- /category nav -->



            <!--href="productos.php?listar"-->



            <!-- menu nav -->



            <div class="menu-nav text-center">



                <span class="menu-header">Menu <i class="fa fa-bars"></i></span>



                <ul class="menu-list ">


<!-- administracion -->
                    <li><a href="admin/">Inicio</a></li>



                    <li class="header-btns dropdown default-dropdown"><a  class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">Productos</a>



                        <div class="custom-menu menuUsuario">



                            <ul>



                                    <li><a href="admin/productos">Listado</a></li>



                                    <li><a href="admin/productos/nuevo-producto">Nuevo</a></li>



                            </ul>



                        </div>



                    </li>



                    <li class="header-btns dropdown default-dropdown"><a  href="admin/usuarios" >Usuarios</a></li>



                    <li class="header-btns dropdown default-dropdown"><a  href="admin/comentarios" >Comentarios</a></li>
                </ul>



            </div>



            <!-- menu nav -->



        </div>



    </div>



    <!-- /container -->



</div>